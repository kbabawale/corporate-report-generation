...
$objPHPExcel->getProperties()->setTitle("Johnson & Johnson Report".date('Y-m-d')); 
$objPHPExcel->getProperties()->setSubject("Johnson & Johnson Report".date('Y-m-d')); 
$objPHPExcel->getProperties()->setDescription("Johnson & Johnson Report generated automatically"); 

// Select current sheet 
$objPHPExcel->setActiveSheetIndex(0);
// Rename sheet
$objPHPExcel->getActiveSheet()->setTitle('Johnson & Johnson Report');
...
// save to a file
$objWriter->save('Reports/J_and_JReport'.date('Y-m-d').'.xlsx'); 


//check if theres a new file generated.
if(file_exists('Reports/J_and_JReport'.date('Y-m-d').'.xlsx')){
    return true;
}else{
    return false;
}
