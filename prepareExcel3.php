...
$objPHPExcel->getProperties()->setTitle("Mars & Wrigley Report".date('Y-m-d')); 
$objPHPExcel->getProperties()->setSubject("Mars & Wrigley Report".date('Y-m-d')); 
$objPHPExcel->getProperties()->setDescription("Mars & Wrigley Report generated automatically"); 

// Select current sheet 
$objPHPExcel->setActiveSheetIndex(0);
// Rename sheet
$objPHPExcel->getActiveSheet()->setTitle('Mars & Wrigley Report');
...
$xls_filename = 'Mars_and_Wrigley'.date('Y-m-d').'.xls'; // Define Excel (.xls) file name
...
FROM ba_calls_update a
      ...
      and a.user_name = 'Nwaizugbe Chinwendu'
      ORDER BY a.rdate
";
...
// save to a file
$objWriter->save('Reports/Mars_and_Wrigley'.date('Y-m-d').'.xlsx'); 


//check if theres a new file generated.
if(file_exists('Reports/Mars_and_Wrigley'.date('Y-m-d').'.xlsx')){
    return true;
}else{
    return false;
}
